...
module.exports = {
  config: {
    ...
    checkDocExecutablPath: {
      type: 'string',
      title: 'Betty documentation check script path (kernel-doc.pl)',
      default: '~/Betty/kernel-doc.pl'
    }
  },
  ...
  lint(textEditor) {
    ...
    this.executablePath !== '') {
      ...
      this.styleLines = helpers.exec(command, parameters)
        .then(function(output) {
          ...
          this.checkDocExecutablPath !== '') {
            ...
            this.docLines = helpers.exec(command, parameters)
              .then(function(output) {
                regex = /^[\w\/]*\.\w:(\d+):\s((?:error|warning)):\s(.+)$/;
                lines = output.split(/\n/);
                for (var i = 0; i < lines.length; ++i) {
                  line = lines[i];
                  if ((match = regex.exec(line)) != null) {
